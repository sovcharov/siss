...
import { MyFileService } from './services/file.service.js';
const myFileService = new MyFileService();
import { MyFunctions } from './services/functions.service.js';
...
app.get('/api/logInUser/:email/:password/:captcha/:companyId', function(req, res) {
  ...
  // })
  const data = `secret=${myNodeConfig.recaptchaSecretKey}&response=${req.params.captcha}`;
  const options = {
