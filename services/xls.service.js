...
export class MyXLService {
  ...
  constructor(
   ...
   createXLPrice(data, callback) {
    ...
    let xlData = [["id","name","manufacturer", "main number", "all numbers", "price", "stock msk", "stock spb", "transit", "picref", "url", "weight", priceUpdatedOnInfo]];
    ...
    for (let i = 0; i < data.length; i += 1) {
      ...
      xlData[xlData.length] = [data[i].id,`${data[i].description} ${data[i].comment}`,`${data[i].manufacturer}`, `${data[i].numberMain}`, `${data[i].numbersString}`, data[i].price, `${data[i].msk}`, `${data[i].stock}`, `${data[i].ordered}`, `${data[i].img}`, `${data[i].url}`, `${data[i].weight}`];
